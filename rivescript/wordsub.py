...
class WordSub(dict):
    ...
    def _update_regex(self):
        ...
        """
        self._regex = re.compile("|".join(map(self._wordToRegex, self.keys())), re.U)
...
if __name__ == "__main__":
    ...
    inStr = "He said he'd like to go with she"
    outStr = "She said she'd like to go with he"
    if subber.sub(inStr) == outStr: print "Test #2 PASSED"    
    else: print "Test #2 FAILED: '%s'" % subber.sub(inStr)

    import lang
    inStr = u"我们怎么这么讨厌你们!"
    print subber.sub(lang.split_zh(inStr))
    inStr = u"我名字叫帅哥"
    print subber.sub(lang.split_zh(inStr))
